...
# Main loop for capturing video frames and processing eye coordinates
while True:
    ret, frame = cap.read()
    frame_count += 1
    ...
    left_pupil_coords, right_pupil_coords = (None, None), (None, None)

    for face in faces:
        landmarks = predictor(gray, face)

        left_pupil_coords = get_pupil_coordinates(
            (36, 37, 38, 39, 40, 41), landmarks, gray
        )
        right_pupil_coords = get_pupil_coordinates(
            (42, 43, 44, 45, 46, 47), landmarks, gray
        )

        if left_pupil_coords != (None, None):
            cv2.circle(frame, left_pupil_coords, 2, (0, 255, 0), -1)
        if right_pupil_coords != (None, None):
            cv2.circle(frame, right_pupil_coords, 2, (0, 255, 0), -1)

        # Calculate the average of both eyes' horizontal and vertical coordinates
        if left_pupil_coords != (None, None) and right_pupil_coords != (None, None):
            avg_x = (left_pupil_coords[0] + right_pupil_coords[0]) / 2
            avg_y = (left_pupil_coords[1] + right_pupil_coords[1]) / 2
        else:
        ...
        df = pd.concat(
            [
                ...
                pd.DataFrame(
                    {
                        "Frame": [frame_count],
                        "Left Eye H": [left_pupil_coords[0]],
                        "Left Eye V": [left_pupil_coords[1]],
                        "Right Eye H": [right_pupil_coords[0]],
                        "Right Eye V": [right_pupil_coords[1]],
                        "Avg H": [avg_x],
                        "Avg V": [avg_y],
                    }
                ),
...
# Function to detect Fixations using averaged coordinates
...
df_with_fixations.to_csv(fixations_output_file_path, index=False)
print(f"Eye coordinates with averaged fixations saved to {fixations_output_file_path}")


# Function to detect Saccades using averaged coordinates
...
saccades_avg_df.to_csv(saccades_avg_output_file_path, index=False)
